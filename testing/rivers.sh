...
do
	wget -O "$gauge".txt "https://nwis.waterdata.usgs.gov/wv/nwis/uv/?cb_00060=on&cb_00065=on&format=rdb&site_no="$gauge"&period=&begin_date=1950-11-01&end_date=2018-12-03"
	sed -n '17p' "$gauge".txt >> flow.txt
	cut -d$'\t' -f 3,5 "$gauge".txt>> flows_out.txt
